...
void comparador_thread(void) {
    ...
    while (1) {
        ...
        } else if (count < 16 && count > 7) {  // Compara o 2º byte (STX)
        ...
        else if (count > 15 && count < 21) {  // Guarda o ID do usuário transmissor 
            id_buf[count3] = variavel;
            count3++;
            count++;
        }

        else if (count > 20 && count < 24) {  // Guarda o tamanho da mensagem
        ...
        else if ((count > tam_msg + 23)  && (count < tam_msg + 31)) {  // Compara o último byte (end)
